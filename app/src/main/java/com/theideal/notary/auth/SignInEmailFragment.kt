...
class SignInEmailFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        signInEmailViewModel.navToForgetPassword.observe(viewLifecycleOwner){
            if (it){
                findNavController().navigate(SignInEmailFragmentDirections.actionSignInMailFragmentToForgetPasswordFragment())
                signInEmailViewModel.doneNavToForgetPassword()
            }
        }
        signInEmailViewModel.navToRegister.observe(viewLifecycleOwner){
            if (it){
                findNavController().navigate(SignInEmailFragmentDirections.actionSignInMailFragmentToRegisterFragment())
                signInEmailViewModel.doneNavToRegister()
            }
        }
